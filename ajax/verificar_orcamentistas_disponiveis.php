...
header('Content-Type: application/json');
$resposta = ['status' => 'erro', 'mensagem' => '', 'orcamentistas' => []];
...
try {
    ...
    $orcamentistas_ocupados = [];
    
    // 1. Verificar orçamentistas com agendamentos neste horário
    if ($tem_agendamentos) {
        ...
        $stmt = $pdo->prepare("SELECT DISTINCT instalador_id FROM agendamentos 
                         ...
                         ((
                            ...
                            -- Modificado para permitir agendamento no exato fim do anterior
        ...
        $orcamentistas_ocupados = $stmt->fetchAll(PDO::FETCH_COLUMN);
    }
    
    // 2. Verificar orçamentistas com indisponibilidades registradas na tabela colaborador_agenda
    $data_apenas = $data_hora_inicio->format('Y-m-d');

    // Consultar orçamentistas indisponíveis em colaborador_agenda
    $stmt_indisponibilidade = $pdo->prepare("SELECT DISTINCT colaborador_id FROM colaborador_agenda 
                               ...
                               AND (
                                   -- Modificado para permitir agendamento no exato fim do período indisponível
                                   (hora_inicio < :hora_fim AND hora_fim > :hora_inicio)
                               )");
    ...
    // Adicionar orçamentistas indisponíveis ao array de ocupados
    $indisponiveis = $stmt_indisponibilidade->fetchAll(PDO::FETCH_COLUMN);
    if (!empty($indisponiveis)) {
        $orcamentistas_ocupados = array_merge($orcamentistas_ocupados, $indisponiveis);
        
        // Remover duplicatas
        $orcamentistas_ocupados = array_unique($orcamentistas_ocupados);
    }
    ...
    // Adicionar orçamentistas com indisponibilidade recorrente ao array de ocupados
    $indisponiveis_recorrentes = $stmt_recorrente->fetchAll(PDO::FETCH_COLUMN);
    if (!empty($indisponiveis_recorrentes)) {
        $orcamentistas_ocupados = array_merge($orcamentistas_ocupados, $indisponiveis_recorrentes);
    ...
    if (!empty($orcamentistas_ocupados)) {
        $sql .= " AND id NOT IN (" . implode(',', $orcamentistas_ocupados) . ")";
    }
